...
> **Note**: The data model includes 11 entities. The `ServiceCredential` entity (Section 10) stores encrypted backend service tokens with `.env` fallback. All other entities are unchanged from the spec.
...
| `deployed` → `modified` | External config change detected | Device config diverged from last deployed profile |
...
| has many | `DeploymentRecord` | one-to-many | Deployment history for this device |
...
- `is_active`: Inactive profiles cannot be targeted in new deployments but remain in history
...
- `content`: Must be valid Jinja2 syntax (validated on save)
...
An immutable log entry capturing a configuration deployment event: who triggered it, which templates were deployed, target devices, timing, and outcome.
...
| `git_branch` | `String(128)` | Not null | Local | Git branch deployed from (e.g., `main`, `staging`) |
| `preview_output` | `Text` | Nullable | Local | Generated config preview (for FR-009) |
| `error_message` | `Text` | Nullable | Local | Error details on failure |
| `config_diff` | `Text` | Nullable | Local | Sections added/modified/removed by merge |
...
- `status`: Must be one of allowed values (see state transitions)
...
pending → in_progress → deployed
    ...
    │
    └→ cancelled
    └→ approved (Admin approval for production branch)
```
...
| `pending`/`approved` → `in_progress` | Pipeline starts | CI/CD job begins execution |
| `in_progress` → `deployed` | NETCONF commit success | Config applied atomically to device |
| `in_progress` → `failed` | NETCONF/pipeline error | Atomic rollback on device; error logged |
...
| belongs to | `Device` | many-to-one | Target device of deployment |
...
```
pending → generated → delivered → completed
    │             │
    │             └→ failed
    │
    └→ cancelled
...
| `generated` → `delivered` | Device fetches ZTP URL | ZTP artifacts served to booting device |
| `delivered` → `completed` | Device reports successful boot | Device is onboarded; `Device.config_status` → `onboarded` |
| `generated`/`delivered` → `failed` | Boot error or timeout | ZTP process failed; error logged |
...
> **Note**: The `DeviceOverride` entity is logically represented by `ConfigurationVariable` records with `scope='device'`. This entity section documents the conceptual model and any override-specific metadata beyond key-value pairs.
...
| `id` | `Integer` | PK, auto-increment | Local | Internal primary key |
| `device_id` | `Integer` | FK → `Device.id`, not null | Local | Target device |
| `profile_id` | `Integer` | FK → `ConfigurationProfile.id`, nullable | Local | Profile being overridden (for context) |
| `variables` | `JSON` | Not null; default `{}` | Local | Override key-value pairs |
| `git_path` | `String(512)` | Not null | Local | Path to `host_vars/<hostname>.yml` |
| `created_at` | `DateTime` | Not null, server default | Local | Record creation timestamp |
| `updated_at` | `DateTime` | Not null, auto-update | Local | Last modification timestamp |

#### Validation Rules

- `variables`: Must be a valid JSON object with string keys and YAML-serializable values
- `git_path`: Must match pattern `host_vars/.+\.yml`
- `device_id` + `profile_id`: Unique constraint (one override set per device-profile pair)
- Override variables merge with profile group_vars at render time; override values win on key collision

#### Relationships

| Relationship | Target | Multiplicity | Description |
|---|---|---|---|
| belongs to | `Device` | many-to-one | Device receiving overrides |
| references | `ConfigurationProfile` | many-to-zero-or-one | Profile context for the overrides |
...
| `token_encrypted` | `Text` | Not null | AES-256 encrypted API token |
...
5. Flags devices missing from NetBox as `stale` (soft delete candidate)
...
|---|---|---|
| `main` | Production configuration | Requires admin approval to merge |
| `staging` | Testing / preview branch | Editors can push directly |
| `working/<user>/<description>` | Auto-commit working branches | Auto-created by web UI on edits |
...
| `deployment_record` | `(device_id, started_at DESC)` | Deployment history per device |
